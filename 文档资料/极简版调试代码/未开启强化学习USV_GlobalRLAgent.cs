...
public class USV_GlobalRLAgent : Agent
{
    ...
    // 勾上 = 随机走（出第三条对比曲线）
    // 不勾 = 正常训练你的PPO
    public bool useRandomActionsInsteadOfRL = true; // 核心开关

    public override void OnActionReceived(ActionBuffers actions)
    {
        // ===================== 【新增：未开启强化学习（随机策略）】 =====================
        if (useRandomActionsInsteadOfRL)
        {
            float randomForward = UnityEngine.Random.Range(-1f, 1f);
            float randomTurn = UnityEngine.Random.Range(-1f, 1f);

            // 直接执行随机动作，跳过模型
            MoveAgentContinuous(randomForward, randomTurn);
